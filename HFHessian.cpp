...
EigenMatrix RKSH(
	...
	for (int x=0;x<3*natoms;x++)
		for (int y=0;y<3*natoms;y++)
			hessian1(x,y)+=2*(dxn[y]*fskeletons[x]).trace()-(wxn[y]*ovlgrads[x]).trace();
	...
	//std::cout<<hessian1-hessian1.transpose()<<std::endl;
	...
	tmp=hessiang2.transpose();
	EigenMatrix diagonalg2(hessiang2.diagonal().asDiagonal());
	hessiang2+=tmp-diagonalg2;
	duration=std::chrono::system_clock::now()-small_start;
	if (output) std::printf("%f s\n",duration.count());
	...
	/*
	if (output) std::printf("| Calculating first-order two-electron perturbation contributions ... ");
	small_start=std::chrono::system_clock::now();
	hessianj=EigenZero(3*natoms,3*natoms);
	hessiank=EigenZero(3*natoms,3*natoms);
	libint2::initialize();
	for (short int s1=0;s1<(short int)obs.size();s1++){
	 atomlist[0]=shell2atom[s1];
	 const short int bf1_first=shell2bf[s1];
	      ...
	      for (short int f3=0;f3!=n3;f3++){
	       ...
	       for (short int f4=0;f4!=n4;f4++,f1234++){
	        ...
	        if (bf2<=bf1 && bf3<=bf1 && bf4<=((bf1==bf3)?bf2:bf3)){
	         ...
	         double tmp=114514;
		 int xpert=1919810;
		 EigenMatrix dyn=EigenZero(nbasis,nbasis);
	         for (int p=0,pt=0;p<4;p++) for (int t=0;t<3;t++,pt++){
	          tmp=abcd_deg*buf_vec[pt][f1234];
	          xpert=3*(atomlist[p])+t;
	          for (int ypert=0;ypert<3*natoms;ypert++){
	           dyn=dxn[ypert];
	           hessianj(xpert,ypert)+=d(bf1,bf2)*tmp*dyn(bf3,bf4);
	           hessianj(xpert,ypert)+=d(bf3,bf4)*tmp*dyn(bf1,bf2);
	           if (kscale>0){
	            hessiank(xpert,ypert)+=d(bf1,bf3)*tmp*dyn(bf2,bf4);
	            hessiank(xpert,ypert)+=d(bf2,bf4)*tmp*dyn(bf1,bf3);
	            hessiank(xpert,ypert)+=d(bf1,bf4)*tmp*dyn(bf2,bf3);
	            hessiank(xpert,ypert)+=d(bf2,bf3)*tmp*dyn(bf1,bf4);
	           }
	          }
	         }
	        }
	...
	libint2::finalize();
	hessiank*=0.5*kscale;
	EigenMatrix hessiangp=hessianj-0.5*hessiank;
	duration=std::chrono::system_clock::now()-small_start;
	if (output) std::printf("%f s\n",duration.count());
	//std::cout<<hessiangp<<std::endl;
	//std::cout<<"fuck"<<std::endl;
	//std::cout<<hessian1+hessiangp<<std::endl;
	//std::cout<<"fuck"<<std::endl;
	//std::cout<<hessian1+0.5*hessiangp<<std::endl;
	//std::cout<<"fuck"<<std::endl;
	//std::cout<<0.5*hessian1+hessiangp<<std::endl;
	*/
