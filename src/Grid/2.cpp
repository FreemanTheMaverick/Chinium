...
#include "../Macro.h"
#include "../Multiwfn.h"
#include "../Integral/Macro.h"
#include "sphere_lebedev_rule.hpp"

int SphericalGridNumber(std::string grid,std::vector<MwfnCenter>& centers,const bool output){
	int ngrids=0;
	__Z_2_Name__
	for (int iatom=0;iatom<centers.size();iatom++){
		int ngroups;
		std::string path=std::getenv("CHINIUM_PATH");
		path+="/Grids/";
		std::ifstream gridfile(path+"/"+grid+"/"+Z2Name[centers[iatom].Index]+".grid");
...
void SphericalGrid(std::string grid,std::vector<MwfnCenter>& centers,
		...
		for (int igroup=0;igroup<ngroups;igroup++){
			...
			for (int ishell=0;ishell<nshells;ishell++,ishell_total++){
				...
				for (int ipoint=0;ipoint<npoints;ipoint++){
					const double x=lebedev_xs[ipoint]*ri+centers[iatom].Coordinates[0];
					const double y=lebedev_ys[ipoint]*ri+centers[iatom].Coordinates[1];
					const double z=lebedev_zs[ipoint]*ri+centers[iatom].Coordinates[2];
					const double lebedev_w=4*M_PI*lebedev_ws[ipoint];
					double unnorm_becke_w_total=0;
					double unnorm_becke_w=0;
					for (int jatom=0;jatom<centers.size();jatom++){
						double unnorm_becke_wj=1;
						const double xj=centers[jatom].Coordinates[0];
						const double yj=centers[jatom].Coordinates[1];
						const double zj=centers[jatom].Coordinates[2];
						for (int katom=0;katom<centers.size();katom++){
							if (jatom==katom) continue;
							const double xk=centers[katom].Coordinates[0];
							const double yk=centers[katom].Coordinates[1];
							const double zk=centers[katom].Coordinates[2];
...
void GetAoValues(std::vector<MwfnCenter>& centers, libint2::BasisSet& obs,
...
void Multiwfn::GenerateGrid(std::string grid, int order, const bool output){
	...
	path += "/Grids/" + grid + "/";
	__Make_Basis_Set__
	this->NumGrids = SphericalGridNumber(grid, this->Centers, 1);
	//if (output) std::cout << "Number of grid points ... " << this->NumGrids << std::endl;
	...
	auto start = __now__;
	SphericalGrid(grid, this->Centers, this->Xs, this->Ys, this->Zs, this->Ws, 1);
