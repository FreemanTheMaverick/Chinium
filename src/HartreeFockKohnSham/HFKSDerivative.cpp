...
void Multiwfn::HFKSGradient(int output){
	...
	if ( this->XC.XCcode == 0 ) return;

	std::vector<int> bf2atom = {};
	int icenter = 0;
	for ( MwfnCenter& center : this->Centers ){
		for ( int jbasis = 0; jbasis < center.getNumBasis(); jbasis++ )
			bf2atom.push_back(icenter);
		icenter++;
	}

	EigenMatrix xcg = EigenZero(this->getNumCenters(), 3);
	...
	long int tmp = 0;
	while ( tmp < this->NumGrids ){
		if ( tmp + ngrids_per_batch < this->NumGrids )
			batch_tails.push_back(tmp + ngrids_per_batch);
		else batch_tails.push_back(this->NumGrids);
	...
	std::vector<std::vector<double*>> batch_gds(this->getNumCenters());
	std::vector<std::vector<double*>> batch_gd1xs(this->getNumCenters());
	std::vector<std::vector<double*>> batch_gd1ys(this->getNumCenters());
	std::vector<std::vector<double*>> batch_gd1zs(this->getNumCenters());
	for ( int icenter = 0; icenter < this->getNumCenters(); icenter++ )
		batch_gds[icenter] = batch_gd1xs[icenter] = batch_gd1ys[icenter] = batch_gd1zs[icenter] = {nullptr, nullptr, nullptr};
