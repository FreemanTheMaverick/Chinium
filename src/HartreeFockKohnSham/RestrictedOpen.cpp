...
std::tuple<double, EigenVector, EigenMatrix> RestrictedOpenRiemann(
	...
	> dfunc_newton = [&](std::vector<EigenMatrix> Cprimes_, int order){
		...
		if ( order == 2 ) He = [nd, ns, Z, Fdprime_, Fsprime_, Cdprime_, Csprime_, &int4c2e, nthreads](EigenMatrix vprime){
...
std::tuple<double, EigenVector, EigenMatrix> RestrictedOpenRiemannARH(
	...
	// ARH hessian related
	std::deque<EigenMatrix> Ddprimes;
	std::deque<EigenMatrix> Dsprimes;
	...
	> dfunc_newton = [&](std::vector<EigenMatrix> Cprimes_, int order){
		...
		// ARH hessian related
		Ddprimes.push_back(Ddprime_);
		Dsprimes.push_back(Dsprime_);
		Fdprimes.push_back(Fdprime_);
		Fsprimes.push_back(Fsprime_);
		if ( Ddprimes.size() > 20 ){
			Ddprimes.pop_front();
			Dsprimes.pop_front();
			Fdprimes.pop_front();
		...
		if ( order == 2 ){
			...
			const int size = (int)Ddprimes.size() - 1;
			const int nbasis = Z.rows();
			std::vector<EigenMatrix> Dddiff(size, EigenZero(nbasis, nbasis));
			std::vector<EigenMatrix> Dsdiff(size, EigenZero(nbasis, nbasis));
			std::vector<EigenMatrix> Fddiff(size, EigenZero(nbasis, nbasis));
			std::vector<EigenMatrix> Fsdiff(size, EigenZero(nbasis, nbasis));
			for ( int i = 0; i < size; i++ ){
				Dddiff[i] = Ddprimes[i] - Ddprime_;
				Dsdiff[i] = Dsprimes[i] - Dsprime_;
				Fddiff[i] = Fdprimes[i] - Fdprime_;
				Fsdiff[i] = Fsprimes[i] - Fsprime_;
			}
			EigenMatrix T = EigenZero(size, size);
			for ( int i = 0; i < size; i++ ) for ( int j = i; j < size; j++ ){
				T(i, j) = T(j, i) = Dddiff[i].cwiseProduct(Dddiff[j]).sum() + Dsdiff[i].cwiseProduct(Dsdiff[j]).sum();
			}
			const EigenMatrix Tinv = T.inverse();
			He = [nd, ns, size, Dddiff, Dsdiff, Fddiff, Fsdiff, Tinv, Cdprime_, Csprime_, Fdprime_, Fsprime_, nthreads](EigenMatrix vprime){
				EigenMatrix vdprime = vprime.leftCols(nd);
				EigenMatrix vsprime = vprime.rightCols(ns);
				EigenMatrix Ddprime = Cdprime_ * vdprime.transpose();
				EigenMatrix Dsprime = Csprime_ * vsprime.transpose();
				Ddprime += Ddprime.transpose().eval();
				Dsprime += Dsprime.transpose().eval();
				std::vector<double> TrDsv(size, 0);
				EigenMatrix HDd = EigenZero(Ddprime.rows(), Ddprime.cols());
				EigenMatrix HDs = EigenZero(Dsprime.rows(), Dsprime.cols());
				for ( int j = 0; j < size; j++ )
					TrDsv[j] += Dddiff[j].cwiseProduct(Ddprime).sum() + Dsdiff[j].cwiseProduct(Dsprime).sum();
				for ( int i = 0; i < size; i++ ){
					double coeff = 0;
					for ( int j = 0; j < size; j++ ){
